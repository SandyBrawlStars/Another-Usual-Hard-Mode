...
ZombieDefinition gZombieDefs[NUM_ZOMBIE_TYPES] = {  
    ...
    { ZOMBIE_SQUASH_HEAD,       REANIM_ZOMBIE,              3,      14,     10,     2000,   _S("SQUASH_ZOMBIE")},
    { ZOMBIE_TALLNUT_HEAD,      REANIM_ZOMBIE,              5,      54,     10,     2000,   _S("TALLNUT_ZOMBIE")},
    { ZOMBIE_CABBAGE_HEAD,      REANIM_ZOMBIE,              2,      52,     10,     2000,   _S("CABBAGE_ZOMBIE") },
    { ZOMBIE_REPEATER_HEAD,      REANIM_ZOMBIE,              5,      24,     10,     2000,   _S("REPEATER_ZOMBIE") },
    { ZOMBIE_SUNFLOWER_HEAD,      REANIM_ZOMBIE,              5,      19,     10,     2000,   _S("SUNFLOWER_ZOMBIE") },
};

ZombieVariantDefinition gZombieVariantDefs[NUM_ZOMBIE_VARIANTS] = {
    {ZOMBIE_VARIANT_DASHER,          _S("DASHER"),            ZOMBIE_NORMAL},
    {ZOMBIE_VARIANT_GHOST,           _S("GHOST"),             ZOMBIE_NORMAL},
    {ZOMBIE_VARIANT_ENDER,           _S("ENDER"),             ZOMBIE_TRAFFIC_CONE},
    {ZOMBIE_VARIANT_GARLIC,          _S("GARLIC"),            ZOMBIE_TRAFFIC_CONE},
    {ZOMBIE_VARIANT_JACKVAULTER,     _S("JACKVAULTER"),       ZOMBIE_POLEVAULTER},
    {ZOMBIE_VARIANT_BUFFVAULTER,     _S("BUFFVAULTER"),       ZOMBIE_POLEVAULTER},
    {ZOMBIE_VARIANT_DEFENDER,     _S("DEFENEDER"),       ZOMBIE_PAIL},
    ...
    {ZOMBIE_VARIANT_RAMSTAR,          _S("RAMSTAR"),            ZOMBIE_FOOTBALL},
};
...
void Zombie::ZombieInitialize(int theRow, ZombieType theType, bool theVariant, Zombie* theParentZombie, int theFromWave)
{
    ...
    case ZombieType::ZOMBIE_NORMAL:  
        ...
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_DASHER);
        }
        else if (RandRangeInt(1, 6) == 6 && mBoard && mBoard->mCurrentWave > 2)
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_GHOST);
        }
        break;
    ...
    case ZombieType::ZOMBIE_TRAFFIC_CONE:  
        ...
        mHelmHealth = 470;
        if (mBoard && mApp->mGameMode == GAMEMODE_CHALLENGE_WAR_AND_PEAS_3 && mFromWave != -5)
        {
            if (RandRangeInt(1, 2) == 2 && IsOnBoard())
            {
                mBoard->AddZombieInRow(ZOMBIE_PEA_HEAD, mRow, -1);
                mBodyHealth = 1;
                TakeBodyDamage(1, 0U);
                mPosX += 300.0f;
            }
            ...
            {
                StartVariant(ZombieVariant::ZOMBIE_VARIANT_GARLIC);
            }
            break;
        }
        if (RandRangeInt(1, 8) == 8 && mBoard && mBoard->mCurrentWave > 3)
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_ENDER);
        }
    ...
    case ZombieType::ZOMBIE_PAIL:  
        ...
        mHelmHealth = 1200;
        if (RandRangeInt(1, 3) == 3 && mBoard && mBoard->mCurrentWave > 3)
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_DEFENDER);
        }
        else if (RandRangeInt(1, 3) == 3 && mBoard && mBoard->mCurrentWave > 5)
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_BOOSTER);
        }
        break;
    ...
    case ZombieType::ZOMBIE_FOOTBALL:  
        ...
        if (RandRangeInt(1, 3) == 3 && mBoard && mBoard->mCurrentWave > 6)
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_RAMSTAR);
        }
        break;
    ...
    case ZombieType::ZOMBIE_POLEVAULTER:  
        ...
        if (RandRangeInt(1, 4) == 4 && mBoard && mBoard->mCurrentWave > 5)
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_JACKVAULTER);
        }
        else if (RandRangeInt(1, 3) == 3 && mBoard && mBoard->mCurrentWave > 4)
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_BUFFVAULTER);
        }
        break;
    ...
    case ZombieType::ZOMBIE_PEA_HEAD:  
    {
        ...
        mPhaseCounter = 150;
        mVariant = false;
        if (mBoard && mApp->mGameMode == GAMEMODE_CHALLENGE_WAR_AND_PEAS_3)
        {
            if (mFromWave != -5)
            {
                if (RandRangeInt(1, 8) == 8 && mBoard)
                {
                    StartVariant(ZombieVariant::ZOMBIE_VARIANT_SNOWPEA);
                }
            }
            break;
        }
        ...
        else if (RandRangeInt(1, 3) == 3 && mBoard && mBoard->mCurrentWave > 3)
        {
            StartVariant(ZombieVariant::ZOMBIE_VARIANT_CURSESHOOTER);
        }
        break;
    }
...
void Zombie::UpdateZombiePolevaulter()
{
    if (mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_PRE_VAULT && mHasHead && mZombieHeight == ZombieHeight::HEIGHT_ZOMBIE_NORMAL)
    {
        ...
        {
            if (mBodyHealth < mBodyMaxHealth && mZombieAge % 30 == 0)
            {
                mBodyHealth += 1;
            }
            mVelX += 0.001f;
        ...
        Plant* aPlant = FindPlantTarget(ZombieAttackType::ATTACKTYPE_VAULT);
        if (aPlant && (mSummonCounter == 1 || mVariantType != ZombieVariant::ZOMBIE_VARIANT_JACKVAULTER))
        {
            ...
            mHasObject = false;
            if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_JACKVAULTER)
            {
                Plant* aTarget = nullptr;
                ...
                while (mBoard->IteratePlants(aPlant))
                {
                    if (aPlant->mRow == mRow && !aPlant->NotOnGround() && !aPlant->IsSpiky())
                    {
                        if (aTarget == nullptr || aPlant->mPlantCol > aTarget->mPlantCol)
                        { 
        ...
        if (aPlant && mSummonCounter == 0 && mVariantType == ZombieVariant::ZOMBIE_VARIANT_JACKVAULTER)
        {
            mSummonCounter = 1;
            int aChoice = 0;
            if (mRow == 0)
...
void Zombie::UpdateZombiePeaHead()
{
    ...
    else if (mPhaseCounter == 0)
    {
        ...
        ProjectileType aProjectileType = ProjectileType::PROJECTILE_ZOMBIE_PEA;
        ProjectileType aHypnoProjectileType = ProjectileType::PROJECTILE_PEA;
        if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_SNOWPEA)
        {
            aProjectileType = PROJECTILE_ZOMBIE_SNOWPEA;
            aHypnoProjectileType = PROJECTILE_SNOWPEA;
        }
        if (mMindControlled)  
        {
            aOriginX += 90.0f * mScaleZombie;
            Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, aHypnoProjectileType);
            aProjectile->mDamageRangeFlags = 1;
            if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_CURSESHOOTER)
            {
                aProjectile->mPoisonOverride = 100;
            }
        }
        else
        {
            Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, aProjectileType);
            aProjectile->mMotionType = ProjectileMotion::MOTION_BACKWARDS;
            if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_CURSESHOOTER)
            {
                aProjectile->mCurseOverride = 300;
            }
        }
...
void Zombie::UpdateZombieSquashHead()
{
    ...
    if (mZombiePhase == ZombiePhase::PHASE_SQUASH_RISING)
    {
        ...
        if (mMindControlled)
        {
            aHeadReanim->SetPosition(mPosX + aPosX + 35.0f, mPosY + aPosY - 21.0f);
        }
    ...
    if (mZombiePhase == ZombiePhase::PHASE_SQUASH_FALLING)
    {
        ...
        if (mPhaseCounter == 2)
        {
            if (mMindControlled)  
            {
                int aDamageRangeFlags = 13U;
                Rect aAttackRect = Rect(mX + 20, mY, mWidth - 35, mHeight);
                ...
                if (mVariantType == ZOMBIE_VARIANT_BURNINGSQUASH)
                {
                    ...
                    while (mBoard->IterateZombies(aZombie))
                    {
                        if (aZombie->mRow == mRow && aZombie->mMindControlled != mMindControlled)
                        {
                            aZombie->TakeDamage(100, 9U);
                        }
                    }
                }
            }
            else
            {
                ...
                while (mBoard->IterateZombies(aZombie))
                {
                    int aGridX = mBoard->PixelToGridXKeepOnBoard(mX + 30, mY);
                    int aZombieGridX = mBoard->PixelToGridXKeepOnBoard(aZombie->mX + 30, aZombie->mY);
                    if (aZombie->mRow == mRow && (aGridX - aZombieGridX) <= 1 && (aGridX - aZombieGridX) >= 0 && aZombie->mMindControlled)
                    {
                        if ((aGridX - aZombieGridX) == 1)
                        {
                            aZombie->TakeDamage(900, 18U);
                            continue;
                        }
                        aZombie->TakeDamage(1800, 18U);
                    }
    ...
    if (mZombiePhase == ZombiePhase::PHASE_SQUASH_DONE_FALLING && mPhaseCounter == 0)
    {
        ...
        if (mVariantType == ZOMBIE_VARIANT_DRAINSQUASH)
        {
            ...
            while (mBoard->IterateZombies(aZombie))
            {
                ...
                if (aZombie->mMindControlled != mMindControlled) continue;
            ...
            if (mMindControlled) aZombie->StartMindControlled();
...
void Zombie::Update()
{
    ...
    int aMouseX = mApp->mWidgetManager->mLastMouseX - 80;
    int aMouseY = mApp->mWidgetManager->mLastMouseY;
    int aGridX = mBoard->PixelToGridXKeepOnBoard(aMouseX, aMouseY);
    int aGridY = mBoard->PixelToGridYKeepOnBoard(aMouseX, aMouseY);
    if (mZBDeluxeMove && mHasHead)
    {
        mRow = aGridY;
        mPosX = aMouseX;
        mPosY = GetPosYBasedOnRow(mRow);
        mRenderOrder = mBoard->MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, mRow, 0);
    }
...
void Zombie::UpdateActions()
{
    ...
    if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_DASHER)
    {
        if (mIsEating && mHasHead && mVelX == 2.0f)
        {
            ...
            if (aZombie)
            {
                ...
                mSummonCounter = 1;
                mApp->PlayFoley(FoleyType::FOLEY_BONK);
                mApp->AddTodParticle(mPosX + 30.0f, mPosY + 70.0f, mRenderOrder - 1, ParticleEffect::PARTICLE_POW);
            }
            else
            {
                Plant* aPlant = FindPlantTarget(ZombieAttackType::ATTACKTYPE_CHEW);
    ...
    if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_RAMSTAR)
    {
        mVelX += 0.005f;
        if (mSummonCounter == 1)
        {
        ...
        UpdateAnimSpeed();
        if (mIsEating&& mHasHead&& mVelX >= 1.0f)
        {
            Zombie* aZombie = FindZombieTarget();
            if (aZombie)
            {
    ...
    if (mZombieType == ZombieType::ZOMBIE_FLAG)
    {
        ...
        while (mBoard->IterateZombies(aZombie))
        {
            if (!aZombie->mDead && (!mMindControlled ? !aZombie->mMindControlled : aZombie->mMindControlled) && aZombie->mZombieType != ZombieType::ZOMBIE_FLAG && aZombie->mBodyHealth > 50)
            {
                ...
                if (mZombieAge % 30 == 0 && aZombie->mBodyHealth < aZombie->mBodyMaxHealth)
                {
    ...
    if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_DEFENDER && mHelmHealth > 0)
    {
        if (mJustGotShotCounter > 24 && mPhaseCounter <= 0)
        {
            Zombie* aZombie = nullptr;
            AddAttachedParticle(40, 40, ParticleEffect::PARTICLE_LANTERN_SHINE);
            while (mBoard->IterateZombies(aZombie))
            {
                int mRowDif = aZombie->mRow - mRow;
                int aGridX = mBoard->PixelToGridXKeepOnBoard(mPosX, mPosY);
                int aZombieGridX = mBoard->PixelToGridXKeepOnBoard(aZombie->mPosX, aZombie->mPosY);
                int aColDif = aZombieGridX - aGridX;
                if (abs(mRowDif) <= 1 && abs(aColDif) <= 1 && aZombie != this && aZombie->mMindControlled == mMindControlled)
                {
                    if (aZombie->mHelmHealth > 0) { aZombie->mHelmHealth += 5; continue; }
                    if (aZombie->mBodyHealth > 0) { aZombie->mBodyHealth += 5; continue; }
                }
            }
            mPhaseCounter = 10;
        }
        if (mJustGotShotCounter == 1)
        {
            AttachmentDetachCrossFadeParticleType(mAttachmentID, ParticleEffect::PARTICLE_LANTERN_SHINE, nullptr);
        }
        
    }
    if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_BOOSTER && mHelmType == HelmType::HELMTYPE_NONE && mZombiePhase == ZombiePhase::PHASE_ZOMBIE_NORMAL)
    {
        float aOriginX = mPosX + 50.0f;
        float aOriginY = mPosY - 44.0f;
        int aTargetX, aTargetY;
        aTargetX = mPosX - 40.0f;
        aTargetY = 0.0f;

        mApp->PlayFoley(FoleyType::FOLEY_BASKETBALL);

        Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, ProjectileType::PROJECTILE_BUCKET);
        float aRangeX = aOriginX - aTargetX - 20.0f;
        float aRangeY = aTargetY - aOriginY;
        if (aRangeX < 40.0f)
        {
        ...
        aProjectile->mVelZ = aRangeY / 120.0f - 7.0f;
        aProjectile->mAccZ = 0.115f;
        aProjectile->mIsZombie = !mMindControlled;
        mZombiePhase = ZombiePhase::PHASE_GHOST_END;
    }
    if (mZombieType == ZombieType::ZOMBIE_SUNFLOWER_HEAD)
    {
        ...
        if (mPhaseCounter == 0)
        {
            ...
            {
                aOriginX += 90.0f * mScaleZombie;
                Coin* aCoin = mBoard->AddCoin(aOriginX, aOriginY, CoinType::COIN_SUN, CoinMotion::COIN_MOTION_LAWNMOWER_COIN);
                mApp->PlayFoley(FoleyType::FOLEY_SUN);
            }
            else
            {
                Coin* aCoin = mBoard->AddCoin(aOriginX, aOriginY, CoinType::COIN_ANTISUN, CoinMotion::COIN_MOTION_LAWNMOWER_COIN);
                mApp->PlayFoley(FoleyType::FOLEY_SUN);
            }
    ...
    if (mZombieType == ZombieType::ZOMBIE_CABBAGE_HEAD)
    {
        ...
        {
            Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
            aHeadReanim->PlayReanim("anim_idle", ReanimLoopType::REANIM_LOOP, 20, 15.0f);
            mApp->PlayFoley(FoleyType::FOLEY_THROW);
            ...
            if (mMindControlled)
            {
                aOriginY -= 10.0f;
                Zombie* aZombie = nullptr;
                int aBestDistance = 1000;
                Zombie* aBestZombie = nullptr;
                while (mBoard->IterateZombies(aZombie))
                {
                    if (!aZombie->mMindControlled && aZombie->mRow == mRow && aZombie->mPosX > mPosX)
                    {
                        int aDistance = aZombie->mPosX - mPosX;
                        if (!aBestZombie || aDistance < aBestDistance)
                        {
                            aBestZombie = aZombie;
                ...
                if (aBestZombie)
                {
                    aTargetX = aBestZombie->mPosX;
                    aTargetY = aBestZombie->mPosY;
                }
                else
                {
                    aTargetX = mPosX + 300.0f;
            ...
            else
            {
                ...
                while (mBoard->IterateZombies(aZombie))
                {
                    if (aZombie->mMindControlled && aZombie->mRow == mRow && aZombie->mPosX < mPosX)
                    {
                        int aDistance = mPosX - aZombie->mPosX;
                        if (!aBestZombie || aDistance < aBestDistance)
                        {
                            aBestZombie = aZombie;
                            aBestDistance = aDistance;
                ...
                if (aPlant)
                {
                    aTargetX = aPlant->mX - 30.0f;
                    aTargetY = aPlant->mY;
                }
                else if (aZombie)
                {
                    aTargetX = aZombie->mPosX + 30.0f;
                    aTargetY = aZombie->mPosY;
                }
                else
                {
                    aTargetX = mPosX - 150.0f;
            ...
            mPhaseCounter = 300;
            if (mApp->mGameMode == GAMEMODE_CHALLENGE_WAR_AND_PEAS_3 && mMindControlled) mPhaseCounter = 250;
        }
    }
    if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_SNOWPEA)
    {
...
void Zombie::DrawReanim(Graphics* g, const ZombieDrawPosition& theDrawPos, int theBaseRenderGroup)
{
    ...
    else if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_RAMSTAR)
    {
        aColorOverride = Color(153, 11, 70, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    ...
    else if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_BOOSTER)
    {
        aColorOverride = Color(242, 83, 15, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    else if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_JACKVAULTER)
    {
        aColorOverride = Color(252, 3, 157, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    else if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_BUFFVAULTER)
    {
        aColorOverride = Color(156, 0, 13, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    else if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_CURSESHOOTER)
    {
        aColorOverride = Color(156, 0, 13, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    ...
    {
        aColorOverride = Color(27, 214, 2, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    ...
    if (mZBDeluxeMove)
    {
        aBodyReanim->mExtraAdditiveColor = Color(255, 255, 255, 230);
...
void Zombie::StartVariant(ZombieVariant theVariant)
{
    mVariantType = theVariant;
    switch (theVariant)
    {

    case ZombieVariant::ZOMBIE_VARIANT_DASHER:
    {
        mVelX = 2.0f;
        UpdateAnimSpeed();
        break;
    ...
    case ZombieVariant::ZOMBIE_VARIANT_RAMSTAR:
    {
        mHelmHealth -= 400;
        mHelmMaxHealth -= 400;
        break;
    ...
    case ZombieVariant::ZOMBIE_VARIANT_GARLIC:
    {
        LoadPlainZombieReanim();
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("anim_head", RENDER_GROUP_HIDDEN);
    ...
    case ZombieVariant::ZOMBIE_VARIANT_SNOWPEA:
    {
        LoadPlainZombieReanim();
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("anim_head", RENDER_GROUP_HIDDEN);
    ...
    case ZombieVariant::ZOMBIE_VARIANT_BOOSTER:
    {
        if (mHelmType == HelmType::HELMTYPE_PAIL)
        {
            mHelmHealth = 150;
            mHelmMaxHealth = 150;
            mBodyHealth = 800;
...
void Zombie::EatPlant(Plant* thePlant)
{
    ...
    {
        thePlant->mPlantHealth -= DAMAGE_PER_EAT * ((mEatPower + (mEatPower * mBucketBoosted)));
    }
...
void Zombie::EatZombie(Zombie* theZombie)
{
    ...
    {
        theZombie->TakeDamage(DAMAGE_PER_EAT, 0U);
    }
    if (mVariantType == ZombieVariant::ZOMBIE_VARIANT_STARPAPER && mZombieAge % 100 < 4)
    {
        for (int i = 0; i < 4; i++)
...
void Zombie::DropLoot()
{
    ...
    {
        if (mBoard->mLevel == 10 && mBoard->mCurrentWave > 5 && !mApp->mPlayerInfo->mHasUnlockedExpansions && mBoard->CountCoinByType(CoinType::COIN_PRESENT_EXPANSION) == 0)
        {
            mApp->PlayFoley(FoleyType::FOLEY_ART_CHALLENGE);
            mBoard->AddCoin(aCenterX, aCenterY, CoinType::COIN_PRESENT_EXPANSION, CoinMotion::COIN_MOTION_COIN);
            return;
        }
        if (mBoard->mLevel == 7 && mBoard->mCurrentWave > 5 && !mApp->mPlayerInfo->mHasUnlockedMinigames && mBoard->CountCoinByType(CoinType::COIN_PRESENT_MINIGAMES) == 0)
        {
            mApp->PlayFoley(FoleyType::FOLEY_ART_CHALLENGE);
            mBoard->AddCoin(aCenterX, aCenterY, CoinType::COIN_PRESENT_MINIGAMES, CoinMotion::COIN_MOTION_COIN);
            return;
        }
    }
...
{
    if (mApp->IsArcadeMode() && IsOnBoard())
    {
        int aMaxPoints = (mBodyMaxHealth + mHelmMaxHealth + mShieldMaxHealth) * 1.5;
        float aPercentage = ClampFloat(1 - (mZombieAge / 6000.0), 0.1, 1);
        mBoard->mArcadePoints += (aMaxPoints * aPercentage) / 4.0;
        mApp->PlayFoley(FoleyType::FOLEY_SPAWN_SUN);
    }
    ...
    if (mShieldType == ShieldType::SHIELDTYPE_DOOR)
    {
        ...
        if (mVariantType == ZOMBIE_VARIANT_BOOSTDOOR)
        {
            ...
            while (mBoard->IterateZombies(aZombie))
            {
                ...
                if (aReanim && aReanim->mReanimationType == ReanimationType::REANIM_ZOMBIE && aZombie->mShieldType != SHIELDTYPE_DOOR && aZombie->mZombieType != ZombieType::ZOMBIE_DOOR && !aZombie->mDead && aZombie->mMindControlled == mMindControlled)
                {
    ...
    if (aDamageIndexAfterDamage != aDamageIndexBeforeDamage)
    {
        ...
        if (mShieldType == ShieldType::SHIELDTYPE_DOOR && aDamageIndexAfterDamage == 1)
        {
            ...
            if (mVariantType == ZOMBIE_VARIANT_BOOSTDOOR)
            {
                ...
                {
                    Reanimation* aReanim = mApp->ReanimationTryToGet(aZombie->mBodyReanimID);
                    if (aReanim && aReanim->mReanimationType == ReanimationType::REANIM_ZOMBIE && aZombie->mShieldType != SHIELDTYPE_DOOR && aZombie->mZombieType != ZombieType::ZOMBIE_DOOR && aZombie->mMindControlled == mMindControlled)
                    {
                        aZombie->mShieldType = ShieldType::SHIELDTYPE_DOOR;
                        aZombie->mShieldHealth = 250; aZombie->mShieldMaxHealth = 250;
                        aZombie->AttachShield();
...
void Zombie::TakeDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    if (mMindControlled)
    {
        SetBit(theDamageFlags, (int)DamageFlags::DAMAGE_BYPASSES_SHIELD, false);
    }
    int aDamageRemaining = theDamage;
    Zombie* aAbsorbZombie = nullptr;
    while (mBoard->IterateZombies(aAbsorbZombie))
    {
        if (aAbsorbZombie->mVariantType == ZOMBIE_VARIANT_ABSORBDOOR && GetCircleRectOverlap(mPosX, mPosY, 110, aAbsorbZombie->GetZombieRect()) && aAbsorbZombie->mShieldHealth > 0 && aAbsorbZombie != this && abs(aAbsorbZombie->mRow - mRow) <= 1 && aAbsorbZombie->mMindControlled == mMindControlled)
        {
    ...
    if (aDamageRemaining > 0 && mShieldType != ShieldType::SHIELDTYPE_NONE && mShieldHealth > 0 && !TestBit(theDamageFlags, (int)DamageFlags::DAMAGE_BYPASSES_SHIELD))
    {
